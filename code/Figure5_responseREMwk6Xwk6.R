...
pdf("figures/Figure5_wk6Xwk6.pdf", height = 10, width = 6)
...
plot(otu_RESPONSEWK6xWK6_roc, add=T, lwd=2, col='red', lty=1)
legend('bottomright', c(sprintf('Remission : AUC=%.3f',otu_remWK6xWK6_roc$auc),sprintf('Response : AUC=%.3f',otu_RESPONSEWK6xWK6_roc$auc)), lwd=2, col=c('royalblue','red'), bty='n', lty=1)
mtext('A', at=1.15, side=3, line=0.1, font=2, cex=2)
...
points(coords(otu_RESPONSEWK6xWK6_roc, x=cutoff, input='threshold', ret='specificity'),coords(otu_RESPONSEWK6xWK6_roc, x=cutoff, input='threshold', ret='sensitivity'), col='red', cex=1.5, pch=10)
...
oturf_tax <- gsub("Candidatus_", "", oturf_tax)
...
mtext('B', at=3.3e-10, side=3, line=0.1, font=2, cex=2)

dev.off()

tiff("figures/Figure5_wk6Xwk6.tiff", height = 10, width = 6, units = "in", res = 300)
