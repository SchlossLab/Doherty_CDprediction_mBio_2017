...
labs <- sapply(strsplit(as.character(row.names(labeled_mean_signif_REMISSwk22_otu_matrix)), " "), 
							 function(x) {
...
cbind(data.frame(labeled_mean_signif_REMISSwk22_otu_matrix, otu = row.names(labeled_mean_signif_REMISSwk22_otu_matrix))) %>%
	gather(Remission, relabund, -otu) %>% ggplot(aes(x=Remission, y=relabund, fill=otu)) +
	geom_bar(position = "dodge",stat="identity") + scale_fill_discrete(labels=labs, breaks = levels(tax_mean_signif_REMISSwk22_otu_matrix$Label)) +
	ylab("Relative Abundance (%)") + xlab("Week 22 Remission")





cbind(data.frame(Vabund_mean_signif_REMISSwk22_otu_matrix, otu = row.names(Vabund_mean_signif_REMISSwk22_otu_matrix))) %>%
	gather(Remission, relabund, -otu) %>% ggplot(aes(x=Remission, y=relabund, fill=otu)) +
	geom_bar(position = "dodge",stat="identity") + 
	scale_fill_discrete(labels=labs, breaks = levels(tax_mean_signif_REMISSwk22_otu_matrix$Label)) +
	ylab("Relative Abundance (%)") + xlab("Week 22 Remission")


cbind(data.frame(Mabund_mean_signif_REMISSwk22_otu_matrix, otu = row.names(Mabund_mean_signif_REMISSwk22_otu_matrix))) %>%
	gather(Remission, relabund, -otu) %>% ggplot(aes(x=Remission, y=relabund, fill=otu)) +
	geom_bar(position = "dodge",stat="identity") + 
	scale_fill_discrete(labels=labs, breaks = levels(tax_mean_signif_REMISSwk22_otu_matrix$Label)) +
	ylab("Relative Abundance (%)") + xlab("Week 22 Remission")
...
cbind(data.frame(Labund_mean_signif_REMISSwk22_otu_matrix, otu = row.names(Labund_mean_signif_REMISSwk22_otu_matrix))) %>%
	gather(Remission, relabund, -otu) %>% ggplot(aes(x=Remission, y=relabund, fill=otu)) +
	geom_bar(position = "dodge",stat="identity") + 
	scale_fill_discrete(labels=labs, breaks = levels(tax_mean_signif_REMISSwk22_otu_matrix$Label)) +
	ylab("Relative Abundance (%)") + xlab("Week 22 Remission")
...
c1c2 <- otu_relab_REMISSwk22[,c(1:2)]
restC <- otu_relab_REMISSwk22[,-c(1:2)]
colnames(restC) <- tax_mean_otu_REMISSwk22_matrix$Label
labeled_otu_relab_REMISSwk22 <- cbind(c1c2, restC)
otuRabREMISSwk22_ggplot <- gather(labeled_otu_relab_REMISSwk22, otu, relabund, one_of(rownames(labeled_mean_signif_REMISSwk22_otu_matrix)))

#phyRabTRTwk22rem_ggplot <- gather(phyRelab_TRTGR_REMISSwk22, phylum, relabund, one_of(row.names(mean_phylum_matrix)[abundant]))
rank <- as.character(levels(tax_mean_signif_REMISSwk22_otu_matrix$Label))
otuRabREMISSwk22_ggplot$otu <- factor(otuRabREMISSwk22_ggplot$otu, levels = rank, labels= rank)  #order the subset dataset by the median abundances
otuRabREMISSwk22_ggplot <- otuRabREMISSwk22_ggplot[order(otuRabREMISSwk22_ggplot$otu),]
tail(colnames(otuRabREMISSwk22_ggplot))

tiff('figures/remisswk22byotu.tiff')
ggplot(otuRabREMISSwk22_ggplot, aes(x=REMISSwk22, y=relabund, fill=otu)) +
...
tiff('figures/remisswk22facetotu.tiff', width = 14, height= 5, units="in", res = 300)
ggplot(otuRabREMISSwk22_ggplot, aes(x=REMISSwk22, y=relabund, fill=REMISSwk22)) + facet_grid(. ~ otu) +
	#stat_summary(fun.y=median, geom="point", aes(fill = otu), position = position_dodge(1)) +
	#stat_summary(fun.data = 'median_hilow', fun.args = (conf.int=0.5), position = position_dodge(1), geom='errorbar', aes(color = otu))+
	...
	ylab("Relative Abundance (%)") + xlab("Week 22 Remission") + theme(legend.position="none")
...
#Abundance stripchart of sig dif otus by REMISSwk22
...
legend('topright', legend=c("Remission Week 22 No", "Remission Week 22 Yes"), pch=c(21, 21), pt.bg=c("orange","royalblue1"), cex=0.7)
