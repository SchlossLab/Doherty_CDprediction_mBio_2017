...
tiff(file='figures/Figure3freqOTUabund.tiff', height=6, width=8, units='in', res=300)
...
for(i in oturf_otus ){
	...
	segments(median(yes_abunds[,i]),index-0.7,median(yes_abunds[,i]),index, lwd=4, col = "black")
	segments(median(no_abunds[,i]),index+0.7,median(no_abunds[,i]),index, lwd=4, col = "black")
	index <- index + 2
}
#oturf_otus<-as.factor(otu_relrespWK22_rf$Xopt)
axis(2, at=seq(1,index-2,2), labels=tax[tax$OTU %in% oturf_otus, 'Label'][as.factor(oturf_otus)], font=3, las=1, line=-0.5, tick=F, cex.axis=0.8)
