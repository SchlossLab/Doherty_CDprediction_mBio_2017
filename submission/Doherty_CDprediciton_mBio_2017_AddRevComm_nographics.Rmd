...
#source("../code/Figure3_REMISSwk6.sig.otu.figure.R")
...
SEX.wilcox.test.by.TRT.data<-na.omit(subset(nao.screen_data, select= c(group, TRTGR, SEX)))
...
set.seed(32016)
SEX.TRTGR.wil <- SEX.cohort.data[,-1]
SEX.TRTGR.wil$TRTGR <- as.numeric(SEX.TRTGR.wil$TRTGR)
SEX.TRTGR.wil$SEX <- as.numeric(SEX.TRTGR.wil$SEX)
SEX.TRTGR.wil.test <- wilcox.test(SEX ~ TRTGR, SEX.TRTGR.wil)
SEX.TRTGR.wil.test.pval <- SEX.TRTGR.wil.test$p.value
...
set.seed(32016)
BCORT.TRTGR.wil <- BCORT.cohort.data[,-1]
BCORT.TRTGR.wil$TRTGR <- as.numeric(BCORT.TRTGR.wil$TRTGR)
BCORT.TRTGR.wil$BCORT <- as.numeric(BCORT.TRTGR.wil$BCORT)
BCORT.TRTGR.wil.test <- wilcox.test(BCORT ~ TRTGR, BCORT.TRTGR.wil)
BCORT.TRTGR.wil.test.pval <- BCORT.TRTGR.wil.test$p.value
...
set.seed(32016)
tissinvol.TRTGR.kw <- tissinvol.cohort.data[,-1]
tissinvol.TRTGR.kw$TRTGR <- as.numeric(tissinvol.TRTGR.kw$TRTGR)
tissinvol.TRTGR.kw$tissinvol <- as.numeric(tissinvol.TRTGR.kw$tissinvol)
tissinvol.TRTGR.kw.test <- kruskal.test(TRTGR ~tissinvol, tissinvol.TRTGR.kw)
tissinvol.TRTGR.kw.test.pval <- tissinvol.TRTGR.kw.test$p.value
tissinvol.TRTGR.kwmc.test <- kruskalmc(TRTGR ~ tissinvol, tissinvol.TRTGR.kw)
...
alltp.ust.RESPy<-alltp.ust[alltp.ust$RESPONSEwk22=="Yes",]
alltp.placplac.RESPy<-alltp.placplac[alltp.placplac$RESPONSEwk22=="Yes",]

#set.seed(32016)
#adonis(alltp_dist ~ TRTGR*RESPONSEwk22, alltime_data, strata = alltime_data$visit)
```

```{r alltp.adiv, include=F}
alltime_data <- alltime_data[alltime_data$TRTGRINDMAN != "Treated_Placebo",]

summary(alltime_data[, c("USUBJID", "visit", "invsimpson", "INDTRTGR", "TRTGRINDMAN", "RESPONSEwk22", "REMISSwk22")])

#change in adiv over time?
wk0.all.adiv <- alltime_data[alltime_data$visit=="Screening", c("USUBJID", "visit", "invsimpson", "INDTRTGR", "TRTGRINDMAN", "RESPONSEwk22", "REMISSwk22")]
wk0.adiv <- wk0.all.adiv
colnames(wk0.all.adiv)[3] <- "Week0adiv"
wk0.all.adiv <- subset(wk0.all.adiv, select=-c(visit))

wk4.all.adiv <- alltime_data[alltime_data$visit=="Week 4", c("USUBJID","visit", "invsimpson", "INDTRTGR", "TRTGRINDMAN", "RESPONSEwk22", "REMISSwk22")]
wk4.adiv <- wk4.all.adiv
colnames(wk4.all.adiv)[3] <- "Week4adiv"
wk4.all.adiv <- subset(wk4.all.adiv, select=-c(visit))

wk6.all.adiv <- alltime_data[alltime_data$visit=="Week 6", c("USUBJID","visit", "invsimpson", "INDTRTGR", "TRTGRINDMAN", "RESPONSEwk22", "REMISSwk22")]
wk6.adiv <- wk6.all.adiv
colnames(wk6.all.adiv)[3] <- "Week6adiv"
wk6.all.adiv <- subset(wk6.all.adiv, select=-c(visit))

wk22.all.adiv <- alltime_data[alltime_data$visit=="Week 22", c("USUBJID","visit", "invsimpson", "INDTRTGR", "TRTGRINDMAN", "RESPONSEwk22", "REMISSwk22")]
...
alltp.treatedrespY <- as.matrix(subset(alltp.treatedrespY, select = -c(USUBJID, INDTRTGR, RESPONSEwk22, REMISSwk22, TRTGRINDMAN)))
...
alltp.treatedrespN <- as.matrix(subset(alltp.treatedrespN,select = -c(USUBJID, INDTRTGR, RESPONSEwk22, REMISSwk22, TRTGRINDMAN)))
...
#source('../code/Figure4_alltp.adivXvisitXtrtgrXRESPONSEwk22.R')
...
alltp.treatedremY <- as.matrix(subset(alltp.treatedremY,select = -c(USUBJID, INDTRTGR, RESPONSEwk22, REMISSwk22, TRTGRINDMAN)))
...
alltp.treatedremN <- as.matrix(subset(alltp.treatedremN,select = -c(USUBJID, INDTRTGR, RESPONSEwk22, REMISSwk22, TRTGRINDMAN)))
...
mdotuvmdWK6resprocsP <- roc.test(mdotu_respWK6_roc, md_respWK6_roc)
otuvmdWK6resprocsP <- roc.test(otu_respWK6_roc, md_respWK6_roc)
otuvmdotuWK6resprocsP <- roc.test(otu_respWK6_roc, mdotu_respWK6_roc)
mdotuvmdWK6resprocsPval <- signif(mdotuvmdWK6resprocsP$p.value, digits=2)
otuvmdWK6resprocsPval <- signif(otuvmdWK6resprocsP$p.value, digits=2)
otuvmdotuWK6resprocsP <- signif(otuvmdotuWK6resprocsP$p.value, digits=3)
```

```{r RESPONSEwk6AUCRFinline, include=F, fig.height=5, fig.width=10}
...
legend('bottomright', c(sprintf('Week 0 OTUs : AUC=%.3f',otu_respWK6_roc$auc), sprintf('Week 0 Clinical Data: AUC=%.3f',md_respWK6_roc$auc), sprintf('Combined: AUC=%.3f',mdotu_respWK6_roc$auc)), lwd=2, col=c('royalblue', 'grey5', 'red'), bty='n', lty=1)

cutoff <- coords(roc=mdotu_respWK6_roc, x='best', best.method='y', ret=c('threshold'))
mdotu_respWK6_spec <- coords(mdotu_respWK6_roc, x=cutoff, input='thr', ret='spec')
mdotu_respWK6_sens <- coords(mdotu_respWK6_roc, x=mdotu_respWK6_spec, input='spec', ret='sens')
mdotu_respWK6_thr <- coords(mdotu_respWK6_roc, x=mdotu_respWK6_spec, input='spec', ret='thr')

points(coords(mdotu_respWK6_roc, x=cutoff, input='threshold', ret='specificity'),coords(mdotu_respWK6_roc, x=cutoff, input='threshold', ret='sensitivity'), col='red', cex=1.5, pch=10)

cutoff <- coords(roc=otu_respWK6_roc, x='best', best.method='y', ret=c('threshold'))
otu_respWK6_spec <- coords(otu_respWK6_roc, x=cutoff, input='thr', ret='spec')
otu_respWK6_sens <- coords(otu_respWK6_roc, x=otu_respWK6_spec, input='spec', ret='sens')
otu_respWK6_thr <- coords(otu_respWK6_roc, x=otu_respWK6_spec, input='spec', ret='thr')
points(coords(otu_respWK6_roc, x=cutoff, input='threshold', ret='specificity'),coords(otu_respWK6_roc, x=cutoff, input='threshold', ret='sensitivity'), col='royalblue', cex=1.5, pch=10)

cutoff <- coords(roc=md_respWK6_roc, x='best', best.method='y', ret=c('threshold'))
md_respWK6_spec <- coords(md_respWK6_roc, x=cutoff, input='thr', ret='spec')
md_respWK6_sens <- coords(md_respWK6_roc, x=md_respWK6_spec, input='spec', ret='sens')
md_respWK6_thr <- coords(md_respWK6_roc, x=md_respWK6_spec, input='spec', ret='thr')

points(coords(md_respWK6_roc, x=cutoff, input='threshold', ret='specificity'),coords(md_respWK6_roc, x=cutoff, input='threshold', ret='sensitivity'), col='black', cex=1.5, pch=10)
...
otu_respWK6xremWK6rocsP <- roc.test(otu_respWK6_roc, otu_remWK6_roc)
otu_respWK6xremWK6rocsPval <- signif(otu_respWK6xremWK6rocsP$p.value, digits=3)

mdotu_respWK6xremWK6rocsP <- roc.test(mdotu_respWK6_roc, mdotu_remWK6_roc)
mdotu_respWK6xremWK6rocsPval <- signif(mdotu_respWK6xremWK6rocsP$p.value, digits=2)
...
mdotuvotuWK6remrocsPval <- signif(mdotuvotuWK6remrocsP$p.value, digits=3)

#source("../code/Figure2_prediction.R")
```
...
```{r RESPONSEwk6Xwk6-AUC-RF, include=F, fig.height=5, fig.width=10}
print("RESPONSEwk6 by AUC-RF \\nRelative response defined as a decrease in CDAI from Week.6 greater than 29.5%.")
set.seed(32016)
levels(trtd.wk6_data$RESPONSEwk6) <- c(0,1)

mdotu_respWK6xWK6 <- trtd.wk6_data[trtd.wk6_data$visit=='Week 6', c('RESPONSEwk6',"AGE", "SEX", "BAMINO", "BANTIBIO", "BCORT", "BIMM", "BL_BMI", "DDUR", "DDUR10", "DDUR15", "tissinvol", "BRSP_TNF", "BL_FLACT", "BL_FCALP", "BL_CRP", "BL_CRPLEV", "BL_FCALPLEV", "BL_FLACTLEV", "BOWSTRICT", "CDSTOOLM_Week.6", "CDPAINM_Week.6", "CDWELBEM_Week.6", "CDARTRTM_Week.6", "CDARALGM_Week.6", "CDIRITM_Week.6", "CDUVEITM_Week.6", "CDERYTHM_Week.6", "CDPYODEM_Week.6", "CDSTOMAM_Week.6", "CDFISURM_Week.6", "CDFISTUM_Week.6", "CDABSCEM_Week.6", "CDOTFISM_Week.6", "CDFEVERM_Week.6", "CDDRGM_Week.6", "CDAMASSM_Week.6", "CDHCTM_Week.6", "CDWEIGHM_Week.6", "invsimpson", colnames(trtd.wk6_data)[grep('Otu[0123456789]', colnames(trtd.wk6_data))])]
#summary(mdotu_respWK6xWK6[,c("RESPONSEwk6")])
mdotu_respWK6xWK6<-na.omit(mdotu_respWK6xWK6)
#summary(mdotu_respWK6xWK6[,c("RESPONSEwk6")])
otu_respWK6xWK6 <- mdotu_respWK6xWK6[, c('RESPONSEwk6', colnames(mdotu_respWK6xWK6)[grep('Otu[0123456789]', colnames(mdotu_respWK6xWK6))])]
#summary(otu_respWK6xWK6[,c("RESPONSEwk6")])
#summary(otu_respWK6xWK6[,c("RESPONSEwk6")])
otu_respWK6xWK6<-na.omit(otu_respWK6xWK6)
summary(otu_respWK6xWK6[,c("RESPONSEwk6")])
set.seed(32016)
otu_respWK6xWK6_rf <- AUCRF(RESPONSEwk6~., data=otu_respWK6xWK6, ranking='MDA', ntree=500, pdel=0.2)
##otu_respWK6xWK6_rf # prints list of optimal OTUs
otu_respWK6xWK6_probs <- predict(otu_respWK6xWK6_rf$RFopt, type='prob')[,2]
otu_respWK6xWK6_roc <- roc(otu_respWK6xWK6$RESPONSEwk6~otu_respWK6xWK6_probs)


md_respWK6xWK6 <- trtd.wk6_data[trtd.wk6_data$visit=='Week 6', c('RESPONSEwk6',"AGE", "SEX", "BAMINO", "BANTIBIO", "BCORT", "BIMM", "BL_BMI", "DDUR", "DDUR10", "DDUR15", "tissinvol", "BRSP_TNF", "BL_FLACT", "BL_FCALP", "BL_CRP", "BL_CRPLEV", "BL_FCALPLEV", "BL_FLACTLEV", "BOWSTRICT", "CDSTOOLM_Week.6", "CDPAINM_Week.6", "CDWELBEM_Week.6", "CDARTRTM_Week.6", "CDARALGM_Week.6", "CDIRITM_Week.6", "CDUVEITM_Week.6", "CDERYTHM_Week.6", "CDPYODEM_Week.6", "CDSTOMAM_Week.6", "CDFISURM_Week.6", "CDFISTUM_Week.6", "CDABSCEM_Week.6", "CDOTFISM_Week.6", "CDFEVERM_Week.6", "CDDRGM_Week.6", "CDAMASSM_Week.6", "CDHCTM_Week.6", "CDWEIGHM_Week.6")]
#summary(md_respWK6xWK6[,c("RESPONSEwk6")])
md_respWK6xWK6<-na.omit(md_respWK6xWK6)
#summary(md_respWK6xWK6[,c("RESPONSEwk6")])
set.seed(32016)
md_respWK6xWK6_rf <- AUCRF(RESPONSEwk6~., data=md_respWK6xWK6, ranking='MDA', ntree=500, pdel=0.3)
##md_respWK6xWK6_rf # prints list of optimal features
md_respWK6xWK6_probs <- predict(md_respWK6xWK6_rf$RFopt, type='prob')[,2]
md_respWK6xWK6_roc <- roc(md_respWK6xWK6$RESPONSEwk6~md_respWK6xWK6_probs)
...
#summary(mdotu_respWK6xWK6[,c("RESPONSEwk6")])
set.seed(32016)
mdotu_respWK6xWK6_rf <- AUCRF(RESPONSEwk6~., data=mdotu_respWK6xWK6, ranking='MDA', ntree=500, pdel=0.2)
#mdotu_respWK6xWK6_rf # prints list of optimal features
mdotu_respWK6xWK6_probs <- predict(mdotu_respWK6xWK6_rf$RFopt, type='prob')[,2]
mdotu_respWK6xWK6_roc <- roc(mdotu_respWK6xWK6$RESPONSEwk6~mdotu_respWK6xWK6_probs)
...
par(mar=c(3,3,2,1), mgp=c(2,0.5,0))
plot(c(1,0),c(0,1), type='l', lty=2, xlim=c(1.01,0), ylim=c(-0.01,1.01), xaxs='i', yaxs='i', ylab='Sensitivity', xlab='Specificity', las=1)
#plot(otu_respWK6xWK6_roc, add=T, lwd=2, col='royalblue', lty=1)
plot(md_respWK6xWK6_roc, add=T, lwd=2, col='grey5', lty=1)
#plot(mdotu_respWK6xWK6_roc, add=T, lwd=2, col='red', lty=1)
title("Predicting Week 6 Response using Week 0 Clinical Data")
legend('bottomright', sprintf('Clinical Data: AUC=%.3f',md_respWK6xWK6_roc$auc), lwd=2, col='grey5', bty='n', lty=1)
cutoff <- coords(roc=mdotu_respWK6xWK6_roc, x='best', best.method='y', ret=c('threshold'))
mdotu_respWK6xWK6_spec <- coords(mdotu_respWK6xWK6_roc, x=cutoff, input='thr', ret='spec')
mdotu_respWK6xWK6_sens <- coords(mdotu_respWK6xWK6_roc, x=mdotu_respWK6xWK6_spec, input='spec', ret='sens')
mdotu_respWK6xWK6_thr <- coords(mdotu_respWK6xWK6_roc, x=mdotu_respWK6xWK6_spec, input='spec', ret='thr')

points(coords(mdotu_respWK6xWK6_roc, x=cutoff, input='threshold', ret='specificity'),coords(mdotu_respWK6xWK6_roc, x=cutoff, input='threshold', ret='sensitivity'), col='red', cex=1.5, pch=10)

cutoff <- coords(roc=otu_respWK6xWK6_roc, x='best', best.method='y', ret=c('threshold'))
otu_respWK6xWK6_spec <- coords(otu_respWK6xWK6_roc, x=cutoff, input='thr', ret='spec')
otu_respWK6xWK6_sens <- coords(otu_respWK6xWK6_roc, x=otu_respWK6xWK6_spec, input='spec', ret='sens')
otu_respWK6xWK6_thr <- coords(otu_respWK6xWK6_roc, x=otu_respWK6xWK6_spec, input='spec', ret='thr')
points(coords(otu_respWK6xWK6_roc, x=cutoff, input='threshold', ret='specificity'),coords(otu_respWK6xWK6_roc, x=cutoff, input='threshold', ret='sensitivity'), col='royalblue', cex=1.5, pch=10)

cutoff <- coords(roc=md_respWK6xWK6_roc, x='best', best.method='y', ret=c('threshold'))
md_respWK6xWK6_spec <- coords(md_respWK6xWK6_roc, x=cutoff, input='thr', ret='spec')
md_respWK6xWK6_sens <- coords(md_respWK6xWK6_roc, x=md_respWK6xWK6_spec, input='spec', ret='sens')
md_respWK6xWK6_thr <- coords(md_respWK6xWK6_roc, x=md_respWK6xWK6_spec, input='spec', ret='thr')

points(coords(md_respWK6xWK6_roc, x=cutoff, input='threshold', ret='specificity'),coords(md_respWK6xWK6_roc, x=cutoff, input='threshold', ret='sensitivity'), col='black', cex=1.5, pch=10)
#dev.off()

#pdf('../data/figures/RSPwk6abunds.#pdf', height=6, width=8)
plot(1, type='n', xlim=c(0.5,2.5), ylim=c(0,1), ylab='', xlab='', xaxt='n', cex.axis=1.2)
stripchart(at=1, md_respWK6xWK6_probs[md_respWK6xWK6$'RESPONSEwk6'==0], vertical=T, method='jitter', jitter=0.4, pch=21, bg='orange', add=T, cex=1.5)
stripchart(at=2, md_respWK6xWK6_probs[md_respWK6xWK6$'RESPONSEwk6'==1], vertical=T, method='jitter', jitter=0.4, pch=21, bg='royalblue', add=T, cex=1.5)
mtext(side=2, text='Probability of Response', line=2.2, cex=0.8)
axis(1, at=c(1,2), labels=c('No', 'Yes'), cex.axis=1.2)
cutoff <- coords(roc=md_respWK6xWK6_roc, x='best', best.method='y', ret=c('threshold')) #gives best point on curve for cutoff
#dev.off()
...
plot(mdotu_respWK6xWK6_roc, add=T, lwd=2, col='red', lty=1)
title("RESPONSEwk6")
legend('bottomright', c(sprintf('OTUs : AUC=%.3f',otu_respWK6xWK6_roc$auc),sprintf('Clinical Data: AUC=%.3f',md_respWK6xWK6_roc$auc),sprintf('Combined: AUC=%.3f',mdotu_respWK6xWK6_roc$auc)), lwd=2, col=c('royalblue','grey5', 'red'), bty='n', lty=1)
#dev.off()

#pdf('../data/figures/RSPwk6abunds.#pdf', height=6, width=8)
plot(1, type='n', xlim=c(0.5,2.5), ylim=c(0,1), ylab='', xlab='', xaxt='n', cex.axis=1.2)
stripchart(at=1, mdotu_respWK6xWK6_probs[mdotu_respWK6xWK6$'RESPONSEwk6'==0], vertical=T, method='jitter', jitter=0.4, pch=21, bg='orange', add=T, cex=1.5)
stripchart(at=2, mdotu_respWK6xWK6_probs[mdotu_respWK6xWK6$'RESPONSEwk6'==1], vertical=T, method='jitter', jitter=0.4, pch=21, bg='royalblue', add=T, cex=1.5)
mtext(side=2, text='Probability of Response', line=2.2, cex=0.8)
axis(1, at=c(1,2), labels=c('No', 'Yes'), cex.axis=1.2)
cutoff <- coords(roc=mdotu_respWK6xWK6_roc, x='best', best.method='y', ret=c('threshold')) #gives best point on curve for cutoff
#dev.off()
...
WK6xWK6rocsP <- roc.test(otu_respWK6xWK6_roc, otu_remWK6xWK6_roc)
WK6xWK6rocsPval <- signif(WK6xWK6rocsP$p.value, digits=2)

#source('../code/Figure5_responseREMwk6Xwk6.R')
```
...
APmd.wk22$TRTGR_RESPONSEwk22 <- as.factor(paste(APmd.wk22$TRTGR, APmd.wk22$RESPONSEwk22))
APmd.wk22$TRTGR_RESPONSEwk22 <- as.factor(paste(APmd.wk22$TRTGR, APmd.wk22$RESPONSEwk22))
...
APmd.wk22$RESPONSEwk4<-as.factor(ifelse(APmd.wk22$percentdelta0_4>=-29.5, print("No"), print("Yes")))
APmd.wk22$RESPONSEwk22<-as.factor(ifelse(APmd.wk22$percentdelta0_6>=-29.5, print("No"), print("Yes")))
APmd.wk22$RESPONSEwk8<-as.factor(ifelse(APmd.wk22$percentdelta0_8>=-29.5, print("No"), print("Yes")))
APmd.wk22$RESPONSEwk22<-as.factor(ifelse(APmd.wk22$percentdelta0_22>=-29.5, print("No"), print("Yes")))
APmd.wk22$RESPONSEwk8_22<-as.factor(ifelse(APmd.wk22$percentdelta8_22>=-29.5, print("No"), print("Yes")))
...
week22.N.trtd.respY <- nrow(week22.trtd[week22.trtd$RESPONSEwk22=='Yes',])
week22.N.trtd.respN <- nrow(week22.trtd[week22.trtd$RESPONSEwk22=='No',])
...
week22.N.plac.respY <- nrow(week22.plac[week22.plac$RESPONSEwk22=='Yes',])
week22.N.plac.respN <- nrow(week22.plac[week22.plac$RESPONSEwk22=='No',])
...
wk22.RESPONSEwk22 <- matrix(c("Week 22", "Response Week 22 (No, Yes)", week22.N.resp.trtd_sum, week22.N.resp.plac_sum, week22.N.all), ncol=5)
colnames(wk22.RESPONSEwk22) <- c("Stool Week", 'Status Week', 'Treated (n)', "Placebo (n)", "Total (n)")
...
wk22_wk22 <- rbind(wk22.RESPONSEwk22, wk22.REMISSwk22)
...
We investigated whether the composition of the baseline fecal microbiota could predict therapeutic remission (CDAI < 150) 6 weeks after induction. To test this hypothesis, we generated Random Forest (RF) models to predict which subjects would be in remission 6 weeks after induction based on the relative abundance of the fecal microbiota at baseline, clinical metadata at baseline, and the combination of microbiota and clinical data. We determined the optimal model based the largest area under the curve (AUC) of the receiver operating characteristic (ROC) curve for the RF model [@baxter_FIT_2016; @calle_aucrf_2011]. Clinical data included components of the CDAI, biomarkers for inflammation, and subject metadata described further in the methods section. We trained these models using `r nrow(nao.screen.trtd)` baseline stool samples from subjects induced with UST; `r trtd.n.YesREMISSwk6.wilcoxtest` of which achieved remission (Table 2). Clinical data alone resulted in an AUC of `r md_remWK6_roc$auc` (specificity = `r md_remWK6_spec`, sensitivity = `r md_remWK6_sens`) (Figure 2A). Using only fecal microbiota data the model had an AUC of `r otu_remWK6_roc$auc` (specificity = `r otu_remWK6_spec`, sensitivity = `r otu_remWK6_sens`). Finally, when combining clinical metadata with the microbiota we achieved an AUC of `r mdotu_remWK6_roc$auc` (specificity = `r mdotu_remWK6_spec`, sensitivity = `r mdotu_remWK6_sens`) for remission 6 weeks after induction. Prediction with clinical metadata alone did not perform as well as models using the baseline fecal microbiome (p = `r round(otuvmdWK6remrocsPval, 3)`) or the combined model (p = `r round(mdotuvmdWK6remrocsPval, 3)`); however, there was not a significant difference between the baseline fecal microbiota model and the combined model (p = `r mdotuvotuWK6remrocsPval`). 

Optimal predictors were determined based on their mean decrease in accuracy (MDA) in the ability of the model to classify remission from active CD (Figure 2B). The majority of OTUs identified as optimal predictors in our model for remission had low abundance. However, two OTUs were differentially abundant for subjects in remission 6 weeks after induction. The relative abundance of *Escherichia/Shigella* (OTU1) was lower in subjects in remission 6 weeks after induction (median = `r round(quantile(trtd.REMwk6.YES.relabund$Otu00001, type = 2, .50), 3)`%, IQR = `r  round(quantile(trtd.REMwk6.YES.relabund$Otu00001, type = 2, .25), 3)`-`r sprintf("%.2f", round(quantile(trtd.REMwk6.YES.relabund$Otu00001, type = 2, .75), 3))`) compared to subjects with active CD (median = `r round(quantile(trtd.REMwk6.NO.relabund$Otu00001, type = 2, .50), 3)`%, IQR = `r  round(quantile(trtd.REMwk6.NO.relabund$Otu00001, type = 2, .25), 3)`-`r round(quantile(trtd.REMwk6.NO.relabund$Otu00001, type = 2, .75), 3)`).  Also, the relative abundance of *Faecalibacterium* (OTU7) was not only higher in subjects in remission 6 weeks after induction (median = `r round(quantile(trtd.REMwk6.YES.relabund$Otu00007, type = 2, .50), 3)`%, IQR = `r  round(quantile(trtd.REMwk6.YES.relabund$Otu00007, type = 2, .25), 3)`-`r round(quantile(trtd.REMwk6.YES.relabund$Otu00007, type = 2, .75), 3)`) than subjects with active CD (median = `r round(quantile(trtd.REMwk6.NO.relabund$Otu00007, type = 2, .50), 3)`%, IQR = `r  sprintf("%.2f", round(quantile(trtd.REMwk6.NO.relabund$Otu00007, type = 2, .25), 3))`-`r sprintf("%.2f", round(quantile(trtd.REMwk6.NO.relabund$Otu00007, type = 2, .75), 3))`), but it was also present prior to the start of UST treatment in every subject who was in remission 6 weeks after induction. 

**Association of baseline microbial signatures with treatment response**

To test whether the composition of the baseline fecal microbiota could predict therapeutic response (CDAI decrease ${\geq}$ 100 points or remission) 6 weeks after induction, we again used RF models to classify responders from non-responders 6 weeks after induction (Table 2). Clinical data alone resulted in an AUC of `r md_respWK6_roc$auc` (specificity = `r sprintf("%.3f", md_respWK6_spec)`, sensitivity = `r md_respWK6_sens`) (Figure 2C). Using only microbiota data, the model predicted response with an AUC of `r otu_respWK6_roc$auc` (specificity = `r otu_respWK6_spec`, sensitivity = `r otu_respWK6_sens`). When combining clinical metadata with the microbiome, the model predicted response with an AUC of `r mdotu_respWK6_roc$auc` (specificity = `r mdotu_respWK6_spec`, sensitivity = `r mdotu_respWK6_sens`). 

The microbiota model was significantly better able to predict response than the metadata alone (p = `r otuvmdWK6resprocsPval`), whereas this was not true for the combined model (p = `r mdotuvmdWK6resprocsPval`). Additionally, the combined model and the fecal microbiota model were not significantly different in their ability to predict response (p = `r otuvmdotuWK6resprocsP`). Optimal predictors were again determined based on their MDA in the ability of the model to classify response (Figure 2D). Also, the baseline combined model was significantly better at classifying remission compared to response (p = `r mdotu_respWK6xremWK6rocsPval`), whereas this was not true for the fecal microbiota model (p = `r otu_respWK6xremWK6rocsPval`).

**Comparison of baseline microbiota based on clinical outcome**

As the RF models identified OTUs abundant across this cohort that were important in classification of outcome, we further investigated differences in the baseline microbiota to assess whether they could serve as potential biomarkers for successful UST treatment. We compared the baseline microbiota of all `r nrow(APmd_shared[APmd_shared$visit=="Screening",])` subjects who provided a baseline sample based on treatment group and treatment outcome 6 weeks after induction to assess diversity measures (Table 2). There was no significant difference in diversity based on response 6 weeks after induction, however the baseline ${\beta}$-diversity was significantly different by response (p = `r wk6RSPbdivpval`). No phyla were significantly different by treatment and response (Fig. S1) and no OTUs were significantly different based on UST response or among subjects receiving placebo for induction, regardless of response and remission status. 

Subjects in remission 6 weeks after induction with UST had significantly higher baseline ${\alpha}$-diversity based on the inverse Simpson diversity index than subjects with active CD (respective median values = `r REMwk6.YES.invsimp` (IQR = `r  Yes.inv.25quantile`-`r Yes.inv.75quantile`), `r REMwk6.NO.invsimp` (IQR = `r  No.inv.25quantile`-`r No.inv.75quantile`), p = `r wk6remadivpval`). The baseline community structure was also significantly different based on remission status in subjects 6 weeks after induction (p = `r all.REMISSwk6.adonis.pval`). Finally, 2 OTUs were significantly more abundant in subjects in remission 6 weeks after induction compared to subjects with active CD: *Bacteroides* (OTU19) (p = `r trtd.REMISSwk6.Otu19p`) and *Faecalibacterium* (OTU7) (p = `r round(trtd.REMISSwk6.Otu7p, 3)`) (Figure 3).  

**Variation in the baseline microbiota is associated with variation in clinical phenotypes**

Based on the associations we identified between baseline microbial diversity and response, we hypothesized that there were associations between the microbiota and clinical variables at baseline that could support the use of the microbiota as a non-invasive biomarker for disease activity [@tedjo_CDactivity_2016]. To test this hypothesis, we compared the baseline microbiota with clinical data at baseline for all `r nrow(APmd_shared[APmd_shared$visit=="Screening",])` samples provided at baseline (Supplemental Table 1). We observed small, but significant correlations for lower ${\alpha}$-diversity correlating with higher CDAI (${\rho}$ = `r signif(cor.cdai$estimate, digits = 3)`, p = `r tbl1$Alpha.Diversity..p.value.[tbl1$Clinical.Variable=="CDAI"]`), higher frequency of loose stools per week (${\rho}$ = `r signif(cor.stool$estimate, digits = 3)`, p = `r tbl1$Alpha.Diversity..p.value.[tbl1$Clinical.Variable=="Loose Stool Frequency (per week)"]`), and longer disease duration (${\rho}$ = `r signif(cor.DDUR$estimate, digits = 3)`, p = `r tbl1$Alpha.Diversity..p.value.[tbl1$Clinical.Variable=="Disease Duration (years)"]`). Corticosteroid use was associated with `r round(median(YesBCORT.wilcoxtest$invsimpson)/median(NoBCORT.wilcoxtest$invsimpson), 2)` times higher ${\alpha}$-diversity (p = `r tbl1$Alpha.Diversity..p.value.[tbl1$Clinical.Variable=="Corticosteroid Use (Y/N)"]`). No significant associations were observed between ${\alpha}$-diversity and CRP, fecal calprotectin, or fecal lactoferrin. However, the ${\beta}$-diversity was significantly different based on CRP (p = `r tbl1$Beta.Diversity..p.value.[3]`), fecal calprotectin (p = `r tbl1$Beta.Diversity..p.value.[4]`), and fecal lactoferrin (p = `r tbl1$Beta.Diversity..p.value.[5]`). The ${\beta}$-diversity was also significantly different based on weekly loose stool frequency (p= `r tbl1$Beta.Diversity..p.value.[2]`), age (p = `r tbl1$Beta.Diversity..p.value.[8]`), the tissue affected (p = `r tbl1$Beta.Diversity..p.value.[tbl1$Clinical.Variable=="Tissue Involvement"]`), corticosteroid use (p =`r tbl1$Beta.Diversity..p.value.[tbl1$Clinical.Variable=="Corticosteroid Use (Y/N)"]`), and disease duration (p = `r tbl1$Beta.Diversity..p.value.[tbl1$Clinical.Variable=="Disease Duration (years)"]`). No significant differences in ${\alpha}$ or ${\beta}$ diversity were observed for BMI, weight, or sex.

**The diversity of the microbiota changes following UST therapy**

We tested whether treatment with UST altered the microbiota by performing a Friedman test comparing ${\alpha}$-diversity, based on the inverse Simpson diversity index, at each time point within each treatment group based on the subject's response 22 weeks after induction. We included `r (nrow(alltp.ust)/4)` subjects induced and maintained with UST (`r (nrow(alltp.ust.RESPyes)/4)` responders, `r (nrow(alltp.ust.RESPno)/4)` non-responders) and `r (nrow(alltp.placplac)/4)` subjects induced and maintained with placebo (`r (nrow(alltp.placplac.RESPyes)/4)` responders, `r (nrow(alltp.placplac.RESPno)/4)` non-responders), who provided samples at every time point (Figure 1). We saw no significant difference in the ${\alpha}$-diversity over time in subjects who did not respond 22 weeks after induction, regardless of treatment, and in subjects who responded to placebo (Figure 4). However, the median ${\alpha}$-diversity of responders 22 weeks after UST induction significantly changed over time (p = `r signif(fried.trtdrespY$p.value, digits = 2)`) having increased from baseline (median = `r median(alltp.treatedrespY$Week0adiv)`, IQR = `r sprintf("%.2f", quantile(alltp.treatedrespY$Week0adiv, 0.25))` - `r quantile(alltp.treatedrespY$Week0adiv, .75)`) to 4 weeks after UST induction(median = `r median(alltp.treatedrespY$Week4adiv)`, IQR = `r quantile(alltp.treatedrespY$Week4adiv, .25)` - `r quantile(alltp.treatedrespY$Week4adiv, .75)`), decreased from 4 to 6 weeks after induction (median = `r median(alltp.treatedrespY$Week6adiv)`, IQR = `r quantile(alltp.treatedrespY$Week6adiv, .25)` - `r quantile(alltp.treatedrespY$Week6adiv, .75)`), and was significantly higher than baseline (p < 0.05) at 22 weeks after induction (median = `r median(alltp.treatedrespY$Week22adiv)`, IQR = 
...
The development of predictive models for disease or treatment outcome is anticipated to have a significant impact on clinical decision-making in health care [@Vogenberg_progmods_2009]. These models may help clinicians decide on the correct course of disease treatment or interventions for disease prevention with their patients. Additionally, patients may benefit with more individualized care that may potentially reduce adverse effects and result in faster recovery, reduce expenses from ineffective therapies, or increase quality of life by preventing disease in patients at high risk.
...
We identified several associations between the microbiota and clinical variables that could impact how CD is monitored and treated in the future. Serum CRP, fecal calprotectin, and fecal lactoferrin are widely used as biomarkers to measure inflammation and CD severity. In this study, the microbial community structure was different among subjects based on these markers. These results support the hypothesis that the fecal microbiota could function as a biomarker for measuring disease activity in patients, especially in concert with established inflammatory biomarkers [@tedjo_CDactivity_2016; @boon_fmarkers_2015; @chang_monitoring_2015]. Higher CDAI scores were also associated with lower microbial diversity. This is consistent with other studies on the microbiota in individuals with CD compared to healthy individuals and studies looking at active disease compared to remission [@gevers_pedsCD_2014; @papa_pedsIBD_2012; @tedjo_CDactivity_2016]. However, the CDAI sub score of weekly stool frequency likely drove these differences (Supplementary Table 1), as we did not observe significant associations between microbial diversity and the other quantitative CDAI sub scores. Our observed association between high loose stool frequency and low microbial diversity is consistent with previous studies [@vandeputte_stoolcon_2016]. We also observed differences in the microbial community structure based on disease localization, which is consistent with a study by Naftali et al [@naftali_tissinvol_2016]. Our study also showed that corticosteroid use impacts the composition of the human fecal microbiota, which is consistent with observations in mouse models [@huang_cort_2015]. We also observed that longer disease duration is associated with a reduction in fecal microbial diversity. We hypothesize that prolonged disease duration and the associated inflammation results in the observed decrease in diversity.
...
Following sequence curation using the mothur software package [@schloss_mothur_2009], we obtained a median of `r median(md$shared_count)` sequences per sample (IQR = `r quantile(md$shared_count, .25, type = 2)`-`r quantile(md$shared_count, .75, type = 2)`). Parallel sequencing of a mock community had an error rate of 0.017%. To limit effects of uneven sampling, we rarefied the dataset to 3,000 sequences per sample. Samples from subjects that completed the clinical trial and had complete clinical metadata were included in our analysis. All raw sequence files and a MIMARKS spreadsheet with de-identified clinical metadata have been uploaded into the NCBI Sequence Read Archive (SRP125127) and are available at https://www.ncbi.nlm.nih.gov/bioproject/PRJNA418765. Additonally, detailed and reproducible descriptions of how the data were processed and analyzed can be found at https://github.com/SchlossLab/Doherty_CDprediction_mBio_2017.

####Gut microbiota biomarker discovery and statistical analysis 

R v.3.3.2 (2016-10-31) and mothur were used to analyze the data [@R]. To assess ${\alpha}$-diversity, the inverse Simpson index was calculated for each sample in the dataset. Spearman correlation tests were performed to compare the inverse Simpson index and continuous clinical data. Wilcoxon rank sum tests were performed for pairwise comparisons and Kruskal-Wallis rank sum tests for comparisons with more than two groups [@sokal_biometrystats_1995; @magurran_measuring_2004]. To measure ${\beta}$-diversity, the distance between samples was calculated using the ${\theta}$YC metric, which takes into account the types of bacteria and their abundance to calculate the differences between the communities [@yue_thetaYC_2005]. These distance matrices were assessed for overlap between sets of communities using the non-parametric analysis of molecular variance (AMOVA) test as implemented in the adonis function from the vegan R package (v.`r as.character(packageVersion("vegan"))`) [@oksanen_vegan_2016]. Changes in ${\alpha}$-diversity over time based on week 22 response was assessed using a Friedman test on subjects who provided a sample at each time point [@friedman_1937]. The Friedman test is a function in the stats R package (v.`r as.character(packageVersion('stats'))`). Multiple comparisons following a Friedman test were performed using the friedmanmc function in the pgirmess package (v.`r as.character(packageVersion("pgirmess"))`) [@pgirmess]. Changes in ${\beta}$-diversity over time by treatment group and response were assessed using the adonis function in vegan stratified by subject. We used the relative abundance of each OTU, ${\alpha}$-diversity, age, sex, current medications, BMI, disease duration, disease location, fecal calprotectin, fecal lactoferrin, C-reactive protein, bowel stricture, and CDAI sub scores as input into our RF models constructed with the AUCRF R package (v.`r as.character(packageVersion("AUCRF"))`) [@AUCRF], to identify phylotypes or clinical variables that distinguish between various treatment and response groups, as well as to predict or determine response outcome [@breiman_rf_2001, @calle_aucrf_2011]. Optimal predictors were determined based on their mean decrease in accuracy (MDA) of the model to classify subjects. Differentially abundant OTUs and phyla were selected through comparison of clinical groups using Kruskal-Wallis and Wilcox tests, where appropriate, to identify OTUs/phyla where there was a p-value less than 0.05 following a Benjamini-Hochberg correction for multiple comparisons [@Benjamini_Hochberg_1995].  Other R packages used in our analysis included ggplot2 v.`r as.character(packageVersion("ggplot2"))` [@ggplot2], dplyr v.`r as.character(packageVersion("dplyr"))` [@dplyr], pROC v.`r as.character(packageVersion("pROC"))` [@pROC], knitr v.`r as.character(packageVersion('knitr'))` [@knitr2015], gridExtra v.`r as.character(packageVersion("gridExtra"))` [@gridExtra], devtools v.`r as.character(packageVersion("devtools"))` [@devtools], knitcitations v.`r as.character(packageVersion("knitcitations"))` [@knitcitations], scales v.`r as.character(packageVersion("scales"))` [@scales], tidyr v.`r as.character(packageVersion("tidyr"))` [@tidyr], Hmisc v.`r as.character(packageVersion("Hmisc"))` [@Hmisc], and cowplot v.`r as.character(packageVersion("cowplot"))` [@cowplot]. A reproducible version of this analysis and manuscript are available at https://github.com/SchlossLab/Doherty_CDprediction_mBio_2017.
...
## Tables
**Table 1: Summary of clinical metadata of cohort at baseline**\footnotemark
...
\footnotetext{No significant differences were observed between placebo and treated groups for sex (p = `r signif(SEX.TRTGR.wil.test.pval, digits=2)`), corticosterioid use (p = `r signif(BCORT.TRTGR.wil.test.pval, digits=1)`), or tissue involment (p > `r tissinvol.TRTGR.kwmc.test$ signif.level`).}
...
**Table 3: Summary of microbial associations with remission at baseline and following UST induction in treated subjects**
...
**Figure 4: Change in alpha diversity over time by induction treatment and week 22 response status.** The ${\alpha}$-diversity of `r (nrow(alltp.ust)/4)` subjects induced and maintained with UST and `r (nrow(alltp.placplac)/4)` subjects induced and maintained with placebo was assessed at each time point. Friedman test were performed within each treatment and responder group. Whiskers represent the range and boxes represent the 25-75% interquartile range of the median (black bar). * indicates week 22 is significantly different from baseline (p <0.05).

```{r  Figure4_alltp.adivXvisitXindtrtXrelRSPwk22.pdf}
```
